...
import { RustyRain } from './components/RustyRain';
...
function App() {
      ...
      setOutput([
        <div key="welcome" className="welcome-message">
          ...
          <div className="text-terminal-text">
            <p className="mb-2">Welcome to GitNaut, your terminal-first GitHub intelligence desk!</p>
            <p className="mb-4 text-terminal-muted">
              Type <span className="text-terminal-success">help</span> to see available commands, 
              or start exploring with <span className="text-terminal-success">gitnaut</span>, 
              <span className="text-terminal-success">projects</span>, or  
  ...
  const handleCommand = useCallback(async (command: string, args: string[]) => {
    ...
    setTimeout(() => {
      ...
      if (outputElement && lastOutput instanceof HTMLElement) {
  ...
  const handleMobileKeyPress = (key: string) => {
    ...
    if (key === 'Enter') {
      ...
      const currentValue = inputElement.value;
      const commands = ['help', 'projects', 'skills', 'contact', 'whoami', 'theme', 'clear', 'history', 'gitnaut'];
